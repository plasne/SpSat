        ...
        protected string ValidReferrerDomain(string urlRef)
        {
            if (!string.IsNullOrEmpty(urlRef))
            {
                ...
                if (uriProtocol == "http" || uriProtocol == "https")
                {
                    ...
                    if ((uriHost.StartsWith("pelasne", StringComparison.OrdinalIgnoreCase) &&
                            uriHost.EndsWith("sharepoint.com", StringComparison.OrdinalIgnoreCase)) ||
                        (uriHost.StartsWith("spsat", StringComparison.OrdinalIgnoreCase) &&
                            uriHost.EndsWith(".azurewebsites.net", StringComparison.OrdinalIgnoreCase)) ||
                        uriHost.StartsWith("localhost", StringComparison.OrdinalIgnoreCase))
                    {
